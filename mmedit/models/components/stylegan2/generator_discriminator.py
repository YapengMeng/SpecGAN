...
class StyleGANv2Generator(nn.Module):
    ...
    def __init__(self,
        ...
        # 这里的input是被训练的参数，构造时内容实质上只提供channel，调用时只利用batch重复
        # 4x4 stage
...
@COMPONENTS.register_module()
class StyleGAN2Discriminator_v2(nn.Module):
    """StyleGAN2 Discriminator.
    ...
    def __init__(self, in_size, channel_multiplier=2,
                 mbstd_cfg=dict(group_size=4, channel_groups=1), pretrained=None, bgr2rgb=False):
        super().__init__()
        ...
        c = 0
        for i in range(log_size, 2, -1):
            convs.append(RDB_Conv(inChannels=channels[in_size] + c * 64, growRate=64, kSize=3, downSample=True))
            c = c + 1
        ...
        self.final_conv = ConvDownLayer(512 + 1, channels[4], 3)
        self.final_linear = nn.Sequential(
            EqualLinearActModule(
                channels[4] * 4 * 4,
                channels[4],
...
@COMPONENTS.register_module()
class StyleGAN2Discriminator_v3(nn.Module):
    """StyleGAN2 Discriminator.
    ...
    def __init__(self, in_size, channel_multiplier=2,
        ...
        convs = [ConvDownLayer(3, in_channels, 1)]
        c = 0
        for i in range(log_size, 2, -1):
            convs.append(RDB_Block(in_channels, growRate=64, nConvLayers=4, kSize=3, downSample=True))
            # convs.append(RDB_Conv(inChannels=in_channels + c * 64, growRate=64, kSize=3, downSample=True))
            c = c + 1
...
class RDB_Conv(nn.Module):
    def __init__(self, inChannels, growRate, kSize=3, downSample=False):
        super().__init__()
        ...
        if downSample:
            ...
            self.conv = nn.Sequential(*[
                nn.Conv2d(Cin, G, kSize, padding=(kSize - 1) // 2, stride=2),
...
class RDB_Block(nn.Module):
    def __init__(self, growRate0, growRate, nConvLayers, kSize=3, downSample=False):
        super().__init__()
        G0 = growRate0
        G = growRate
        C = nConvLayers
        ...
        convs = []
        for c in range(C):
            convs.append(RDB_Conv(G0 + c * G, G, kSize, downSample=False))
        self.convs = nn.Sequential(*convs)

        # Local Feature Fusion 这里给每个像素点执行线性融合
        down_sample_stride = 2 if downSample else 1
        self.LFF = nn.Sequential(*[
            nn.Conv2d(G0 + C * G, G0 + C * G, 3, padding=1, stride=down_sample_stride),
            nn.Conv2d(G0 + C * G, G0, 1, padding=0, stride=1)])
        self.skip = nn.Conv2d(G0, G0, 1, padding=0, stride=down_sample_stride)

    def forward(self, x):
        return (self.LFF(self.convs(x)) + self.skip(x)) / math.sqrt(2)
