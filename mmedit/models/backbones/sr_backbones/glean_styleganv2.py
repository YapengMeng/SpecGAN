...
from mmedit.models.common import PixelShufflePack, make_layer, InterpolatePack
...
class GLEANStyleGANv2(nn.Module):
    ...
    Args:
        ...
        img_out_channels (int): Number of channels of the input images. 3 for RGB
            image and 1 for grayscale image. Default: 3.
        rdb_channels (int): Number of channels of the RRDB features.
            Default: 64.
        num_rdb_blocks (int): Number of RRDB blocks in the encoder. Default: 23.
    ...
    def __init__(self,
        ...
        self.generator = build_component(
            dict(
                ...
                bgr2rgb=bgr2rgb))
        # TODO 注意这里generator设置了generator.requires_grad_(False)
        ...
        self.encoder.append(
            nn.Sequential(
                ...
                RDB_branch(num_RDB_block=16, growRate0=64, growRate=64, nConvLayers=8, in_channels=img_in_channels),
                nn.Conv2d(
        ...
        for res in decoder_res:
            ...
            if res < out_size:
                ...
                self.decoder.append(
                    InterpolatePack(
                        in_channels, out_channels, scale_factor=2, upsample_kernel=3))
            else:
                self.decoder.append(
                    nn.Sequential(
    ...
    def forward(self, lq):
        ...
        encoder_features = encoder_features[::-1]  # 倒序排列
...
class RDB_branch(nn.Module):
    def __init__(self, num_RDB_block, growRate0, growRate, nConvLayers, kSize=3, in_channels=64):
        super().__init__()
