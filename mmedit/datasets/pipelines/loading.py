...
class LoadTIFImageFromFile:
    ...
    def __call__(self, results):
        ...
        if self.use_cache:
            ...
            img = gdal_utils.read_full_image(img_path=filepath, scale_factor=1, as_rgb=False,
                                             data_format='NUMPY_FORMAT', normalize=True,
                                             normalize_factor=1, band_idx=None)
        results[self.key] = img
        results[f'{self.key}_path'] = filepath
        results[f'{self.key}_ori_shape'] = img.shape
        if self.save_original_img:
